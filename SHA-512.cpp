...
string decimaltohexadecimal(int64 decimal){
	string EQBIN = bitset<64>(decimal).to_string();
	string hexstring = "";
	string temp;
...
int64 BinarytoDecimal(string bin){
...
int64 rotateRight(int64 x, int n){
	return (x >> n) | (x << (64 - n));
}

int64 shiftRight(int64 x, int n){
		return (x >> n);
}
...
void separator(string getBlock){
	...
	for (unsigned int i = 0; i < getBlock.length(); i += 64, ++chunknum) {
		Message[chunknum] = BinarytoDecimal(getBlock.substr(i, 64));
	}
	for (int g = 16; g < 80; ++g){
		int64 WordA = rotateRight(Message[g - 2], 19)
					^ rotateRight(Message[g - 2], 61)
					^ shiftRight(Message[g - 2], 6);
		int64 WordB = Message[g - 7];
		int64 WordC = rotateRight(Message[g - 15], 1)
					^ rotateRight(Message[g - 15], 8)
					^ shiftRight(Message[g - 15], 7);
...
string SHA512(string myString){
	...
	output << decimaltohexadecimal(A);
	output << decimaltohexadecimal(B);
	output << decimaltohexadecimal(C);
	output << decimaltohexadecimal(D);
	output << decimaltohexadecimal(E);
	output << decimaltohexadecimal(F);
	output << decimaltohexadecimal(G);
	output << decimaltohexadecimal(H);
	return output.str();
}
